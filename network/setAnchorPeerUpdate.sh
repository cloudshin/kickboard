...
#########################################################
######## set anchor peer for Org4 in the channel ########
#########################################################
...
infoln "Modify the configuration to append the anchor peer"
Modify the configuration to append the anchor peer 
jq '.channel_group.groups.Application.groups.Org4MSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.org4.example.com","port": 13051}]},"version": "0"}}' config.json > modified_config.json
{ set +x; } 2>/dev/null
...
configtxlator proto_decode --input config_update.pb --type common.ConfigUpdate --output config_update.json
echo '{"payload":{"header":{"channel_header":{"channel_id":"'$CHANNEL_NAME'", "type":2}},"data":{"config_update":'$(cat config_update.json)'}}}' | jq . > config_update_in_envelope.json
configtxlator proto_encode --input config_update_in_envelope.json --type common.Envelope --output Org4MSPAnchor.tx
{ set +x; } 2>/dev/null


# Anchor peer set for org4 on channel
infoln "Anchor peer set for org4 on channel"
peer channel update -f Org4MSPAnchor.tx -c "${CHANNEL_NAME}" -o localhost:7050  --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$ORDERER_CA"
popd